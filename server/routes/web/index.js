...
module.exports = app => {
    ...
    router.get('/news/init', async (req, res) => {
        ...
        const newsTitles = [
            ...
            "日体验服停机更新公告",
            "新英雄澜CG",
            "】设计、视频、音乐赛道获奖名单——王者荣耀风物志共创大赛",
            "可以跟王者策划零距离“对线”交流的机会，你会……",
            "】云阁绮霞 -王者荣耀风物志插画漫画大赛",
            "｜守护万千星河，云上仙君新装登场！",
            "12月23日全服不停机更新公告",
            "12月22日英雄平衡性调整丨孙策、玄策、达摩等多位英雄增强",
            "参与活动免费解锁KPL限定皮肤个人专属购买6折特权",
            "恭喜DYG夺得KPL秋季赛冠军！冠军回馈来袭，双旦福利开启！",
            "嫦娥皮肤设计创意赏析第二期，女神新装抢先看！"
        ]
    ...
    router.get('/news/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            { $match: { parent: parent._id } },//条件查询类似于where
            //类似于关系型数据库的join左外连接，以它为主体查另一个集合  
            { $lookup: { from: "articles", localField: '_id', foreignField: 'categories', as: 'newsList' } },
            {
                /**
                 *  添加字段(其实在$lookup已经添加了一个字段heroList,但是$lookup里面的是把分类关联的新闻都找了出来，
                 *  如果要显示的并不是所有的新闻，只要显示5个或10个，可以用$lice改heroList字段,把原有的heroList取出5个或10个)
                 */
                $addFields: {//mongodb 3.4版本以上才能用
                    //特殊操作符 $slice表示去取里面的几个（类似数组的slice）后面是一个数组表示多个参数['','']第一个参数要筛的字段，第二个参数表示要几个
    ...
    router.get('/heroes/init', async (req, res) => {
       ...
       for (let cat of rawData) {         
            ...
            cat.heroes = cat.heroes.map(hero => {
                //英雄所属分类，可以用[category._id]，但是MongoDB足够智能（如果传的是一个分类对象，就会把分类的对象的ID提取出来，放到category里），可以直接用[category]
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        //聚合查询
        const cats = await Category.aggregate([
            //$match 先查 parent字段等于parent._id， 条件查询类似于where
            { $match: { parent: parent._id } },//根据parent找到所有子分类，子分类关联lookup找到英雄列表
            //关联查询类似于关系型数据库的join左外连接，以它为主体查另一个集合  
